...
var JunitReporter = function (_events$EventEmitter) {
    ...
    _createClass(JunitReporter, [{
                ...
                for (var _iterator2 = Object.keys(runners)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    ...
                    try {
                        for (var _iterator3 = Object.keys(capabilities.specs)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                            var _specId = _step3.value;

                            var _spec = capabilities.specs[_specId];
                            ...
                            try {
                                for (var _iterator4 = Object.keys(_spec.suites)[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                                    ...
                                    var _suite = _spec.suites[suiteKey];
                                    var _suiteName = this.prepareName(_suite.title);
                                    var _testSuite = builder.testSuite().name(_suiteName).timestamp(_suite.start).time(_suite.duration / 1000).property('specId', _specId).property('suiteName', _suite.title).property('file', _spec.files[0].replace(process.cwd(), '.'));
                                    ...
                                    try {
                                        for (var _iterator5 = Object.keys(_suite.tests)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                                            var testKey = _step5.value;

                                            if (testKey !== 'undefined') {
                                                // fix cucumber hooks crashing reporter
                                                var _test = _suite.tests[testKey];
                                                var _testName = this.prepareName(_test.title);
                                                var testCase = _testSuite.testCase().className(_suiteName).name(_testName).time(_test.duration / 1000);

                                                if (_test.state === 'pending') {
                                                    testCase.skipped();
                                                }
                                                    ...
                                                    if (errorOptions) {
                                                        ...
                                                        try {
                                                            for (var _iterator6 = Object.keys(errorOptions)[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
                                                                ...
                                                                testCase[_key](_test.error[errorOptions[_key]]);
                                                            }
                                                        } catch (err) {
                                                        ...
                                                        // default
                                                        testCase.error(_test.error.message);
                                                    }
                                                    testCase.standardError('\n' + _test.error.stack + '\n');
                                                }

                                                var output = this.getStandardOutput(_test);
        ...
        key: 'testSuiteCapabilitiesToggle',
        value: function testSuiteCapabilitiesToggle(boolean) {
            if (false) {
                return this.testSuite = builder.testSuite().name(suiteName).timestamp(suite.start).time(suite.duration / 1000).property('specId', specId).property('suiteName', suite.title).property('file', spec.files[0].replace(process.cwd(), '.'));
            }
        }
    }, {
        key: 'testCaseClassNameToggle',
        value: function testCaseClassNameToggle(boolean) {
            if (false) {
                return this.testCase = testSuite.testCase().className(suiteName).name(testName).time(test.duration / 1000);
            }
        }
    }, {
